...
(defn pfactors_pl [n pl] (if (= 0 (count pl)) '()

                                         (let [p (first pl) pf (pfactors_pl n (rest pl))] (
        if (= 0 (mod n p)) (conj pf p) pf))))


(defn remults [n l] (cond
                                 (= 0 (count l)) l
...
(defn erato [l] (cond
                             (= 0 (count l)) l
                             :else (let [n (first l) lt (rest l)]
                                    (conj (erato (remults n lt)) n)))
)

(defn coprime_divisor [n p] (if (= 0 (mod n p)) (coprime_divisor (/ n p) p) n))
...
(defn pdivisors [n p] (if (not (= 0 (mod n p))) '(1)
                                      (addp p (pdivisors (/ n p) p))))

(defn all_pdivisors [n pf] (if (= 0 (count pf)) '()
...
(defn each_permissible [n pf] (for [p pf] (filter #(= 1 (mod % p)) (rest (divisors_over (coprime_divisor n p) pf)))))

(defn each_max_divisor [n pf] (map #(loop [v 1] (if (= 0 (mod (/ n v) %)) (recur (* v %)) v)) pf))

(def primes (erato (range 2 100)))

(defn nonnil [l] (if (= 0 (count l)) '() (let [nnlt (nonnil (rest l)) lf (first l)] (if (nil? lf) nnlt (conj nnlt lf)))))

(defn pnonnil [l] (if (= 0 (count l)) () (do (if (not (nil? (first l))) (println (first l))) (pnonnil (rest l)))))
...
(def n47_ans (for [n (range 3 10000 2)]
  (let [pf (pfactors_pl n primes)]
    ...
    (if (= 1 (last_prime_factor n pf))
      (let [ep (each_permissible n pf) pkf (each_max_divisor n pf)]
